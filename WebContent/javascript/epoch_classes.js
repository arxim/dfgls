...
function Epoch(name,mode,targetelement,action,multiselect)
{
	this.state = 0;
	this.name = name;
	...
	if(this.mode == 'popup' && targetelement && targetelement.type == 'text'){ 
		...
		this.eventAction.calendar = this;
		this.eventAction.onclick = function() {this.calendar.show();}; 
		this.tgt.onchange = function() {if(!this.calendar.mousein){this.calendar.hide();}}; 
	}else{
...
Epoch.prototype.calConfig = function () //PRIVATE: initialize calendar variables
{
	...
	this.minDate = new Date(2005,1,0);
	this.maxDate = new Date(2037,1,0);
...
//-----------------------------------------------------------------------------
Epoch.prototype.getRight = function (element) //PRIVATE: returns the absolute Left value of element, in pixels
{
    var oNode = element;
    var iRight = 0;
    
    while(oNode.tagName != 'BODY') {
        iRight += oNode.offsetRight;
        oNode = oNode.offsetParent;        
    }
...
Epoch.prototype.createFooter = function () //PRIVATE: creates the footer of the calendar - goes under the calendar cells
{
	...
	var cancelSelected = document.createElement('input');
	...
	cancelSelected.setAttribute('type','button');
	cancelSelected.setAttribute('value',this.clearbtn_cancel);
	cancelSelected.owner = this;
	cancelSelected.onclick = function() {this.owner.cancelSelections();};
	container.appendChild(clearSelected);
	container.appendChild(cancelSelected);
	return container;
};
//-----------------------------------------------------------------------------
Epoch.prototype.cancelSelections = function ()  //PRIVATE: reset the calendar's selection variables to defaults
{
    this.hide();
};
//-----------------------------------------------------------------------------
...
Epoch.prototype.updatePos = function (target) //PUBLIC: moves the calendar's position to target's location (popup mode only)
{
	...
	this.calendar.style.right = this.getRight(target) + this.rightOffset + 'px'
}
//-----------------------------------------------------------------------------
