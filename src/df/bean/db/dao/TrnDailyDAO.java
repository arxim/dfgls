...
public class TrnDailyDAO {
	...
	private String getTemplateForBasicAllocate(){
		return 	"SELECT TRN_DAILY.INVOICE_NO, TRN_DAILY.INVOICE_DATE, TRN_DAILY.ADMISSION_TYPE_CODE AS ADMISSION_TYPE, TRN_DAILY.TRANSACTION_DATE, "+
				...
				"CASE WHEN TRN_DAILY.DOCTOR_PRIVATE_CODE = 'Y' OR TRN_DAILY.DOCTOR_PRIVATE_CODE = TRN_DAILY.DOCTOR_CODE THEN 'Y' ELSE 'N' END AS PRIVATE_DOCTOR, "+
				"CASE WHEN TRN_DAILY.INVOICE_TYPE = 'ORDER' THEN TRN_DAILY.INVOICE_TYPE ELSE 'EXECUTE' END AS DOCTOR_TREATMENT, TRN_DAILY.AMOUNT_AFT_DISCOUNT, "+
	...
	public boolean rollbackStepCalculate(){
		String sql = "UPDATE TRN_DAILY SET DR_AMT = 0.00, HP_AMT = 0.00, SEQ_STEP = null, OLD_DR_AMT = 0.00, TAX_TYPE_CODE = '', "+
			         ...
			         "NOR_ALLOCATE_AMT = 0.00 WHERE HOSPITAL_CODE = ? AND TRANSACTION_DATE BETWEEN ? AND ? AND ( SEQ_STEP IS NOT NULL AND SEQ_STEP > 0.00 )";
		try{
