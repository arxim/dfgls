...
public class StpMethodAllocMaster extends StpMethodAllocItem {
    ...
    public boolean IsFound(String orderItemCode, String doctorTreatmentCode, String hospitalCode, String doctorCategoryCode, String admissionTypeCode) {
        boolean ret = false;
        String sql = "select * from STP_METHOD_ALLOC_MASTER where " +
    ...
    public StpMethodAllocMaster(String doctorTreatmentCode, String hospitalCode, String doctorCategoryCode, String admissionTypeCode, DBConnection conn) {
//        super("", doctorTreatmentCode, hospitalCode, doctorCategoryCode, admissionTypeCode, conn);
        super();
        this.setDBConnection(conn);
        this.setResultSet(this.getDBConnection().executeQuery("select * from STP_METHOD_ALLOC_MASTER where " +
                                                " DOCTOR_TREATMENT_CODE='" + doctorTreatmentCode + "'" +
                                                " and HOSPITAL_CODE='" + hospitalCode + "'" +
                                                " and DOCTOR_CATEGORY_CODE='" + doctorCategoryCode + "'" +
                                                " and ADMISSION_TYPE_CODE='" + admissionTypeCode + "'"));
    ...
    public StpMethodAllocMaster(String hospitalCode, DBConnection conn) {
        super();
        this.setDBConnection(conn);
        this.setResultSet(this.getDBConnection().executeQuery("select * from STP_METHOD_ALLOC_MASTER where HOSPITAL_CODE='" + hospitalCode + "'"));
            try {
                this.clearAllArry();
    ...
    public boolean IsFound(String orderItemCode, String doctorTreatmentCode, String doctorCategoryCode, String admissionTypeCode) {
        ...
        for (int i=0; i<this.priceArry.size(); i++) {
            if (this.doctorTreatmentCodeArry.get(i).toString().equals( doctorTreatmentCode.toString()) &&
