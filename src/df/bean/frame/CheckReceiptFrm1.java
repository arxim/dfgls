...
import df.bean.process.ProcessCheckReceipt;
...
public class CheckReceiptFrm1 extends CalculatorFrm1 {
    ...
    public CheckReceiptFrm1() {
        try {
//            if (connect())
                jbInit();
        } catch (Exception e) {
            e.printStackTrace();
            DialogBox.ShowError("Error in ImportInvoiceFrm\n"+e.getMessage());
        }
    }
...
//        menuName = passParameter.menuName;
        ...
        scrollPane.setBounds(new Rectangle(10, 180, 115, 215));
        ...
        scrollPaneHospitalCode.setBounds(new Rectangle(10, 85, 115, 60));

       
        //Table Scroll Pane
        String[] columnNames={ "Hospoital", "Invoice No", "Remark" };
        ...
        scrollPaneMessage.setBounds(new Rectangle(135, 65, 595, 330));
        ...
        tblMessage.getColumnModel().getColumn(2).setMinWidth(400);
...
/*        String sql = "select distinct INVOICE_NO from RECEIPT_DETAIL where HOSPITAL_CODE = '" + hospitalCode + "'";
        sql = sql + " and ISLOADED = '" + TCInvoice.STATUS_UNLOADED + "'";
        sql = sql + " and doctor_cure_code not in ";
        sql = sql + " ( SELECT CODE FROM doctor where (DOCTOR.from_date > '" + JDate.getDate() + "'";
        sql = sql + " or DOCTOR.to_date < '" + JDate.getDate() + "')";
        sql = sql + " or DOCTOR.ACTIVE <> '1')"; 
    ...
    private void start_actionPerformed(ActionEvent e) {
        if (connect()) {
            addHospitalCodeToList();
            this.setProcess(new ProcessCheckReceipt(this.conn));
