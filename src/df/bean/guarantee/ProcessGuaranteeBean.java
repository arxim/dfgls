...
public class ProcessGuaranteeBean {
    DBConn cdb;
    String[][] g_setup = null;
    String result = "";
    String month = "";
     ...
     */
    public ProcessGuaranteeBean(DBConn cdb){
        try {
            this.cdb = cdb;
    ...
    public boolean prepareProcess(String month, String year, String hospital_code, String process_type, String user_id){
        ...
        //new method
        System.out.print(Variables.IS_TEST ? "Guarantee Process Type : "+process_type+"\n" : "");
        ...
        if(status && process_type.equals("Backup Tax Before Guarantee")){
        	...
        	status = this.transferAllocateFromDr();
        }
        ...
        if(status && process_type.equals("Guarantee Calculate")){
        	status = this.setGuaranteePaidAmount();
        }
    ...
    private boolean setOrderItemGuarantee(){ //new method
        ...
        try{
            ...
            //System.out.println("setOrderItemGuarantee Error : "+e);
        }
    ...
    private boolean transferAllocateFromDr(){
        ...
        try{
            ...
            for(int i = 0; i<doctor.length; i++){
            	try{
                	if(Double.parseDouble(""+doctor[i][1])<1){
                		doctor[i][1] = "100";
                	}
            	}catch(Exception e){
            		doctor[i][1] = "100";
            	...
            	try{
                	if(Double.parseDouble(""+doctor[i][2])<1){
                		doctor[i][2] = "100";
                	}
            	}catch(Exception e){
            		doctor[i][2] = "100";
            	...
            	try{
            		...
            		cdb.rollDB();
            		System.out.println("Exception Update Over/In Allocate : "+e);
            	}
            }
			...
			if(arr_checklist.length>0){
				...
				for(int i = 0; i < arr_checklist.length; i++){
					...
					if(!arr_checklist[i][0].equals("NORMAL")){
						...
						//do not anything for stp_guarantee table
					}
				}
				}catch(Exception e){
            ...
            try{
                ...
                for(int i = 0; i<g_setup.length; i++){
                    ...
                    try{
                    	...
                    	System.out.println("Exception Update Guarantee Amount (DF or 100%) "+e);
                    	cdb.rollDB();
                    }
                    try{
                ...
                System.out.println("Update Setup Transaction for Guarantee Exception : "+e+" on : "+temp+" sql "+sql_statement);
                result = "Update transaction guarantee error : "+e+" on : "+temp+" sql "+sql_statement;
                cdb.rollDB();
                status = false;
    ...
    /* method don't use
    private boolean transferAllocateFromDr(){ //method unused  	
    	//THIS METHOD UPDATE IN/OVER_ALLOCATE FROM DOCTOR TO STP_GUARANTEE ONLY
    	//Vejthani Set IN/OVER Allocate from Guarantee Setup
        boolean status1 = true;
