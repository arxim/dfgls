...
public class InterfaceExportGLBGHBean {
    ...
    public InterfaceExportGLBGHBean(String yyyy, String mm, String hospital_code){
        conn = new DBConn();
        YYYY = yyyy;
        MM = mm;
    ...
    private String accu602101DeditNoGuanrantee(){
        return "INSERT INTO TEMP_GL(LINE_NO, DOCTOR_CODE, ADMISSION_TYPE_CODE, PATIENT_DEPARTMENT_CODE," +
                ...
                "AND T.ORDER_ITEM_ACTIVE = '1' AND OI.ACCOUNT_CODE <> '602102' "+
                "AND D.DOCTOR_TYPE_CODE NOT LIKE 'G%' ";
                /*
        		"AND ("+
                	"(D.GUARANTEE_START_DATE IS NULL OR D.GUARANTEE_START_DATE = '') OR"+
                	" '"+this.YYYY+this.MM+"01' NOT BETWEEN D.GUARANTEE_START_DATE AND D.GUARANTEE_EXPIRE_DATE" +
                ")";
                */
    }    
    private String accu602102DeditNoGuanrantee(){
        return "INSERT INTO TEMP_GL(LINE_NO, DOCTOR_CODE, ADMISSION_TYPE_CODE,PATIENT_DEPARTMENT_CODE" +
    ...
    private String accu602103Credit(){ //transaction hp_amt
        return "INSERT INTO TEMP_GL(LINE_NO, DOCTOR_CODE, ADMISSION_TYPE_CODE,PATIENT_DEPARTMENT_CODE" +
                ...
                "T.AMOUNT_AFT_DISCOUNT - T.DR_AMT AS HP_AMT, "+
                //"T.HP_AMT, " +
    ...
    private String accuAbsorbDebitGuatantee(){
        return "INSERT INTO TEMP_GL(DOCTOR_CODE, ADMISSION_TYPE_CODE,PATIENT_DEPARTMENT_CODE" +
               ",AMOUNT_AFT_DISCOUNT,ACCOUNT_CODE,RECEIPT_LOCATION,PATIENT_LOCATION,NATIONALITY_CODE," +
               "AMOUNT_SIGN,MM,YYYY,HOSPITAL_CODE,PROCESS) " +
               "SELECT T.DOCTOR_CODE, 'O', "+
               "D.DEPARTMENT_CODE, "+
               //"T.SUM_TAX_406+T.SUM_TAX_402 AS ABSORB, "+ // EDIT ON 2009-09-08
               //--UPDATE 23/08/2010
    ...
    private String gl602101DebitNoGuatantee(){
        return "INSERT INTO TEMP_GL(LINE_NO, DOCTOR_CODE, ADMISSION_TYPE_CODE,PATIENT_DEPARTMENT_CODE" +
                ",AMOUNT_AFT_DISCOUNT,ACCOUNT_CODE,RECEIPT_LOCATION,PATIENT_LOCATION," +
                "NATIONALITY_CODE,AMOUNT_SIGN,MM,YYYY,HOSPITAL_CODE,PROCESS) " +
                ...
                + "     AND (OI.ACCOUNT_CODE <> '602102' OR OI.ACCOUNT_CODE IS NULL) "
    ...
    private String gl602102DebitNoGuatantee(){
        return "INSERT INTO TEMP_GL(LINE_NO,DOCTOR_CODE,ADMISSION_TYPE_CODE,PATIENT_DEPARTMENT_CODE," +
        "AMOUNT_AFT_DISCOUNT,ACCOUNT_CODE,RECEIPT_LOCATION,PATIENT_LOCATION," +
        "NATIONALITY_CODE,AMOUNT_SIGN,MM,YYYY,HOSPITAL_CODE,PROCESS) " +
        "SELECT SD.LINE_NO, SD.DOCTOR_CODE, SD.ADMISSION_TYPE_CODE, " +
        ...
        +" AND DR.IS_HOLD <> 'Y' "
        +" AND SD.ACTIVE = '1' AND SD.ORDER_ITEM_ACTIVE = '1' AND OI.ACCOUNT_CODE = '602102' "
        +" AND SD.COMPUTE_DAILY_DATE is not null AND SD.COMPUTE_DAILY_DATE != ''"
        +" AND (SD.PAY_BY_CASH='Y' OR SD.PAY_BY_AR='Y' OR SD.PAY_BY_DOCTOR='Y' OR SD.PAY_BY_PAYOR='Y' OR SD.PAY_BY_CASH_AR='Y' )"
        +" AND SD.IS_PAID = 'Y'"
        +" AND DR.DOCTOR_TYPE_CODE NOT LIKE 'G%'";
    ...
    private String gl602105DebitGuarantee(){
        return "INSERT INTO TEMP_GL(LINE_NO, DOCTOR_CODE, ADMISSION_TYPE_CODE,PATIENT_DEPARTMENT_CODE" +
                ...
                " 	AND (SD.PAY_BY_CASH='Y' OR SD.PAY_BY_AR='Y' OR SD.PAY_BY_DOCTOR='Y' OR SD.PAY_BY_PAYOR='Y' OR SD.PAY_BY_CASH_AR='Y' )"+
                " 	AND SD.IS_PAID = 'Y'"+
                /*--Comment for update to GL Bank & Cheque 13/10/2009
                " 	AND SD.DOCTOR_CODE IN"+
                //new code for bank transfer only
                "     (SELECT DOCTOR_CODE FROM BANK_TMB_MEDIA_CLEARING WHERE YYYY = '"+this.YYYY+"' AND MM = '"+this.MM+"' ) "+
                --*/
    ...
    private String glAbsorbDebitGuatantee(){
        return "INSERT INTO TEMP_GL(DOCTOR_CODE,ADMISSION_TYPE_CODE,PATIENT_DEPARTMENT_CODE" +
               ...
               "WHERE T.YYYY = '"+this.YYYY+"' AND T.MM = '"+this.MM+"' "+
               //--UPDATE 13/10/2009 FOR INTERFACE GL ALL PAYMENT
               " AND D.PAYMENT_MODE_CODE <> 'U' AND D.PAYMENT_MODE_CODE <> '' AND D.PAYMENT_MODE_CODE IS NOT NULL"+
               " AND D.IS_HOLD <> 'Y'"+
               " AND D.ACTIVE = '1' " +
    ...
    private void insertIntoIntErpGl(){
        ...
        //System.out.println(" Insert into INT_ERP_GL : " + count_insert + ", Accounting Date : "+Accounting_DT+" from process time : "+JDate.getTime());
    }
    public String processAccu(){
        ...
        try{
            ...
            conn.insert(this.accu602101DeditNoGuanrantee());
            conn.insert(this.accu602102DeditNoGuanrantee());
    ...
    public String processGlAccount(){
        ...
        try{
            ...
            conn.insert(this.gl602101DebitNoGuatantee());
            conn.insert(this.gl602102DebitNoGuatantee());
            conn.insert(this.gl602103CreditSharing());
            conn.insert(this.gl602105DebitGuarantee());
