...
public class ProcessHolidayCalculate {
			...
			if(INCLUDE.equals("Y")){
				...
				while(rsMethod2.next()){
					...
					String sqlcommand =
							...
							+"TRN_DAILY.IS_GUARANTEE_FROM_ALLOC = 'Y', "
							//+"TRN_DAILY.NOR_ALLOCATE_PCT ="+rsMethod2.getDouble("NOR_ALLOCATE_PCT")+", "
							+"TRN_DAILY.NOR_ALLOCATE_AMT =0.00, "
							+"TRN_DAILY.DR_AMT = (CASE WHEN TRN_DAILY.COMPUTE_DAILY_USER_ID != 'Employee' THEN TRN_DAILY.AMOUNT_AFT_DISCOUNT * "+rsMethod2.getDouble("NOR_ALLOCATE_PCT")/100+" ELSE (TRN_DAILY.AMOUNT_AFT_DISCOUNT * TRN_DAILY.NOR_ALLOCATE_PCT / 100) * "+rsMethod2.getDouble("NOR_ALLOCATE_PCT")/100+" END), "
							+"TRN_DAILY.DR_TAX_406 = (CASE WHEN TRN_DAILY.COMPUTE_DAILY_USER_ID != 'Employee' THEN TRN_DAILY.AMOUNT_AFT_DISCOUNT ELSE TRN_DAILY.AMOUNT_AFT_DISCOUNT * TRN_DAILY.NOR_ALLOCATE_PCT / 100 END), "
	...
	public boolean dataRollBack(String hospitalcode,String yyyy,String mm){
		...
		String sqlRollbackTrnDaily = 
			"UPDATE TRN_DAILY SET " +
			"TRN_DAILY.COMPUTE_DAILY_USER_ID = HIS_TRN_DAILY.COMPUTE_DAILY_USER_ID, "+
