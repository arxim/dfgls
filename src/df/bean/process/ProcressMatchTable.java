            ...
            for(int i=0; i < arr.length ; i++){
                ...
                if(this.getTypeGuarantee().equals("DLY")){
                    ...
                    if(this.getTypeGuarantee().equals("MLY")){
                        ...
                        for(int k = 0 ; k < arrDt.length ; k++){
                            sqlInsert = "INSERT INTO STP_GUARANTEE(HOSPITAL_CODE, USER_ID, MM, YYYY, GUARANTEE_DR_CODE, GUARANTEE_CODE, ADMISSION_TYPE_CODE, GUARANTEE_TYPE_CODE, IS_GUARANTEE_DAILY, START_DATE, START_TIME, END_DATE, END_TIME, GUARANTEE_AMOUNT, GUARANTEE_EXCLUDE_AMOUNT, GUARANTEE_ALLOCATE_PCT, OVER_ALLOCATE_PCT, GUARANTEE_FIX_AMOUNT, GUARANTEE_INCLUDE_AMOUNT, GL_DEPARTMENT_CODE) " +
                            "VALUES('"+ this.getHpCode() +"','TRANSFER','"+this.getMm()+"','"+ this.getYyyy() +"', '"+ arr[i][1] +"','"+ guarantee_code +"','U','MLD','Y','"+ this.StartDate(arrDt[k]) +"','"+arr[i][4]+"','"+ this.StartDate(arrDt[k]) +"','"+ arr[i][5] +"','"+arr[i][6]+"','"+arr[i][7]+"',"+arr[i][8]+","+arr[i][9]+", '"+arr[i][10]+"', '"+arr[i][11]+"','"+arr[i][12]+"')";
                            try{
                            	conn.insert(sqlInsert);
                            	conn.commitDB();
                            }catch(Exception e){
	                    	...
	                    	sqlInsert = "INSERT INTO STP_GUARANTEE(HOSPITAL_CODE, USER_ID, MM, YYYY, GUARANTEE_DR_CODE, GUARANTEE_CODE, ADMISSION_TYPE_CODE, GUARANTEE_TYPE_CODE, IS_GUARANTEE_DAILY, START_DATE, START_TIME, END_DATE, END_TIME, GUARANTEE_AMOUNT, GUARANTEE_EXCLUDE_AMOUNT, GUARANTEE_ALLOCATE_PCT, OVER_ALLOCATE_PCT, GUARANTEE_FIX_AMOUNT, GUARANTEE_INCLUDE_AMOUNT, GL_DEPARTMENT_CODE) " +
	                    	"VALUES('"+ this.getHpCode() +"','TRANSFER','"+this.getMm()+"','"+ this.getYyyy() +"', '"+ arr[i][1] +"','"+ guarantee_code +"','U','MLY','N','"+ this.StartDate("01") +"','"+arr[i][4]+"','"+ this.EndDate(arr[i][4], arr[i][5], "") +"','"+ arr[i][5] +"','"+arr[i][6]+"','"+arr[i][7]+"',"+arr[i][8]+","+arr[i][9]+", '"+arr[i][10]+"', '"+arr[i][11]+"','"+arr[i][12]+"')";
	                    	try{
	                        	conn.insert(sqlInsert);
	                        	conn.commitDB();
	                        }catch(Exception e){
                    		...
                    		sqlInsert = "INSERT INTO STP_GUARANTEE(HOSPITAL_CODE, USER_ID, MM, YYYY, GUARANTEE_DR_CODE, GUARANTEE_CODE, ADMISSION_TYPE_CODE, GUARANTEE_TYPE_CODE, IS_GUARANTEE_DAILY, START_DATE, START_TIME, END_DATE, END_TIME, GUARANTEE_AMOUNT, GUARANTEE_EXCLUDE_AMOUNT, GUARANTEE_ALLOCATE_PCT, OVER_ALLOCATE_PCT, GUARANTEE_FIX_AMOUNT, GUARANTEE_INCLUDE_AMOUNT, GL_DEPARTMENT_CODE) " +
                    		"VALUES('"+ this.getHpCode() +"','TRANSFER','"+this.getMm()+"','"+ this.getYyyy() +"', '"+ arr[i][1] +"','"+ guarantee_code +"','U','MMY','N','"+ this.StartDate("01") +"','"+arr[i][4]+"','"+ this.EndDate(arr[i][4], arr[i][5], "") +"','"+ arr[i][5] +"','"+arr[i][6]+"','"+arr[i][7]+"',"+arr[i][8]+","+arr[i][9]+", '"+arr[i][10]+"', '"+arr[i][11]+"','"+arr[i][12]+"')";
		                	try{
		                    	conn.insert(sqlInsert);
		                    	conn.commitDB();
		                    }catch(Exception e){
	                    ...
	                    sqlInsert = "INSERT INTO STP_GUARANTEE(HOSPITAL_CODE, USER_ID, MM, YYYY, GUARANTEE_DR_CODE, GUARANTEE_CODE, ADMISSION_TYPE_CODE, GUARANTEE_TYPE_CODE, IS_GUARANTEE_DAILY, START_DATE, START_TIME, END_DATE, END_TIME, GUARANTEE_AMOUNT, GUARANTEE_EXCLUDE_AMOUNT, GUARANTEE_ALLOCATE_PCT, OVER_ALLOCATE_PCT, GUARANTEE_FIX_AMOUNT, GUARANTEE_INCLUDE_AMOUNT, GL_DEPARTMENT_CODE) " +
                        "VALUES('"+ this.getHpCode() +"','TRANSFER','"+this.getMm()+"','"+ this.getYyyy() +"', '"+ arr[i][1] +"','"+ guarantee_code +"','U','STP','N','"+ this.StartDate("01") +"','"+arr[i][4]+"','"+ this.EndDate(arr[i][4], arr[i][5], "") +"','"+ arr[i][5] +"','"+arr[i][6]+"','"+arr[i][7]+"',"+arr[i][8]+","+arr[i][9]+", '"+arr[i][10]+"', '"+arr[i][11]+"','"+arr[i][12]+"')";
                    	try{
                        	conn.insert(sqlInsert);
                        	conn.commitDB();
                        }catch(Exception e){
