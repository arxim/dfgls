...
public class ReportQuery {
    ...
    private String unpaidDeatil(){
    	String q = ""+
    			"SELECT DR.CODE AS PROFILE_CODE, DR.NAME_THAI AS PROFILE_NAME, "+
    			"SD.INVOICE_NO AS 'INVOICE_NO', SD.INVOICE_DATE AS 'INVOICE_DATE', SD.VERIFY_DATE AS 'VERIFY_DATE', "+
    			"SD.VERIFY_TIME AS 'VERIFY_TIME', SD.DOCTOR_CODE AS 'DOCTOR_CODE', SD.YYYY AS 'YYYY', SD.MM AS 'MM', "+
    			"SD.HN_NO AS 'HN_NO', SD.PATIENT_NAME AS 'PATIENT_NAME', SD.ORDER_ITEM_CODE AS 'ORDER_ITEM_CODE', "+
    			"OI.DESCRIPTION_THAI AS 'ORDER_ITEM_DESCRIPT', SD.AMOUNT_AFT_DISCOUNT, SD.DR_AMT AS 'DR_AMT', SD.IS_DISCHARGE_SUMMARY "+
    			...
    			"LEFT OUTER JOIN ORDER_ITEM OI ON (OI.CODE = SD.ORDER_ITEM_CODE AND OI.HOSPITAL_CODE = SD.HOSPITAL_CODE) "+

    			"WHERE SD.HOSPITAL_CODE LIKE '"+this.hospital_code+"' (SD.INVOICE_DATE BETWEEN '00000000' AND '"+this.year+this.month+"31') "+
    			"AND DR.CODE LIKE '"+this.doctor_code+"' AND (SD.YYYY+SD.MM='' OR SD.YYYY+SD.MM >'"+this.year+this.month+"') AND "+
    			"SD.ACTIVE = '1' AND DR.ACTIVE = '1' AND SD.ORDER_ITEM_ACTIVE = '1' AND SD.INVOICE_TYPE <> 'ORDER' AND SD.IS_PAID = 'Y' AND SD.DR_AMT <> 0 "+
    			"ORDER BY PROFILE_CODE,DOCTOR_CODE,IS_DISCHARGE_SUMMARY,INVOICE_DATE,SD.ADMISSION_TYPE_CODE DESC,PAYOR_OFFICE_CODE";
    	logger.info("Unpaid query : "+q);
    	return q;
    }
    ...
    private String ac(){
    	return ""+
    	"SELECT MODULE, DOCTOR_CODE, NAME_THAI, HN_NO, PATIENT_NAME, INVOICE_NO, LINE_NO, INVOICE_DATE, AMOUNT_AFT_DISCOUNT, "+
    	"PAID_AMOUNT, SHARING,ACCOUNT_CODE, PATIENT_DEPARTMENT_CODE, TAX_TYPE_CODE, ONWARD, PAID "+
    	"FROM( "+
    	"SELECT 'DF' AS MODULE, TM.DOCTOR_CODE, DR.NAME_THAI, TM.HN_NO, TM.PATIENT_NAME, TM.INVOICE_NO, TM.LINE_NO, "+
    	"TM.INVOICE_DATE, " +
    	...
    	"CASE WHEN DR.DOCTOR_TYPE_CODE LIKE 'G%' THEN HP.EARNING_ACCOUNT ELSE OI.ACCOUNT_CODE END AS ACCOUNT_CODE, "+
    	"TM.PATIENT_DEPARTMENT_CODE, TM.TAX_TYPE_CODE, TM.IS_ONWARD AS ONWARD, YYYY AS PAID "+
    	"FROM TRN_DAILY TM "+
    	"LEFT OUTER JOIN DOCTOR DR ON TM.DOCTOR_CODE = DR.CODE AND TM.HOSPITAL_CODE = DR.HOSPITAL_CODE "+
    	"LEFT OUTER JOIN ORDER_ITEM OI ON TM.ORDER_ITEM_CODE = OI.CODE AND TM.HOSPITAL_CODE = OI.HOSPITAL_CODE "+
    	"LEFT OUTER JOIN DEPARTMENT DEPT ON TM.PATIENT_DEPARTMENT_CODE = DEPT.CODE AND TM.HOSPITAL_CODE = DEPT.HOSPITAL_CODE "+
    	...
    	"SELECT 'ADD' AS MODULE, TX.DOCTOR_CODE, DR.NAME_THAI, '' AS HN_NO, '' AS PATIENT_NAME, '' AS INVOICE_NO, TX.LINE_NO, "+
    	"'' AS INVOICE_DATE, TX.AMOUNT AS AMOUNT_AFT_DISCOUNT, TX.AMOUNT AS 'PAID_AMOUNT', '0' AS 'SHARING', "+
    	"TX.EXPENSE_ACCOUNT_CODE, TX.DEPARTMENT_CODE, TM.TAX_TYPE_CODE, TM.IS_ONWARD AS ONWARD, YYYY AS PAID "+
    	"FROM TRN_EXPENSE_DETAIL TX "+
    	"LEFT OUTER JOIN DOCTOR DR ON TX.DOCTOR_CODE = DR.CODE AND TX.HOSPITAL_CODE = DR.HOSPITAL_CODE "+
    	"LEFT OUTER JOIN DEPARTMENT DEPT ON TX.DEPARTMENT_CODE = DEPT.CODE AND TX.HOSPITAL_CODE = DEPT.HOSPITAL_CODE "+
    	"WHERE TX.HOSPITAL_CODE = '"+this.hospital_code+"' AND TX.YYYY+TX.MM = '"+this.year+this.month+"' "+
